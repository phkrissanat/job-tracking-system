...
// Stable per-row id. Assigned once (on upload/add) and carried through every
// edit after that, so a row can always be found by id alone — no more
// guessing from its content, which breaks down whenever two rows happen to
// have the same code/details (e.g. duplicate rows with blank fields).
function genRowId(){
  return 'r_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2,9);
}
function withRowIds(rows){
  return (rows||[]).map(function(r){
    return (r && !r._id) ? Object.assign({}, r, { _id: genRowId() }) : r;
  });
}
...
async function pushToCollection(collectionName, rows, srcName){
  ...
  rows = withRowIds(rows); // give every row a stable id, so edits can find it reliably later
...
// One-time (safe to call repeatedly) maintenance pass: gives every existing
// row in "งานปัจจุบัน" a stable _id if it doesn't already have one, so old
// rows — including duplicate/near-identical ones that content-matching
// can't tell apart — become reliably editable right away, without having
// to wait for someone to save each one first. Only touches rows missing an
// id; already-id'd rows and the rest of the row's data are left untouched.
// Returns how many rows were newly stamped.
export async function fbBackfillRowIds(){
  var metaSnap = await getDoc(doc(db,'mold_sync','meta'));
  if(!metaSnap.exists()) return 0;
  var chunkCount = metaSnap.data().chunkCount || 0;
  var stamped = 0;
  for (var i=0; i<chunkCount; i++){
    var ref = doc(db,'mold_sync','chunk_'+i);
    var snap = await getDoc(ref);
    if(!snap.exists()) continue;
    var rows = snap.data().rows || [];
    var changed = false;
    for (var k=0; k<rows.length; k++){
      if(!rows[k] || !rows[k]._id){
        rows[k] = Object.assign({}, rows[k], { _id: genRowId() });
        changed = true; stamped++;
      }
    }
    if(changed) await setDoc(ref, { rows: rows });
  }
  return stamped;
}
...
// ── GROUP / CATEGORY MASTER LIST
// Single doc, same pattern as column_widths/freeze_columns above — small
// enough (a few hundred {group,category} pairs at most) to fit comfortably
// in one document rather than a whole collection.
export async function fbPushGroupMaster(list){
  await setDoc(doc(db,'app_settings','group_category_master'), { list: list, ts: Date.now() });
}
export function fbWatchGroupMaster(callback){
  return onSnapshot(doc(db,'app_settings','group_category_master'), function(snap){
    if(!snap.exists()){ callback({ list: [] }); return; }
    var d=snap.data();
    callback({ list: d.list||[], ts: d.ts||null });
  }, function(err){
    callback({ error: err });
  });
}
...
function subUserCollName(role){
  if(role!=='validators' && role!=='dataSpecialists' && role!=='itemCreators') throw new Error('invalid role: '+role);
  return role;
}
...
export async function fbCheckSubUserLogin(username, password){
  ...
  var roles = [
    ...
    { col: 'itemCreators', role: 'itemCreator' }
  ];
...
export async function fbSubmitTrialResult(code, resultValue, validatorName){
  ...
  return runTransaction(db, async function(tx){
    ...
    // The ISO inspection checklist (form 740-FM-028) requirement is
    // TEMPORARILY DISABLED — the standard form itself isn't finalized yet,
    // so validators can submit a TRIAL result without it for now. Re-enable
    // by uncommenting the block below once the form is settled.
    ...
    // UTC calendar date (DD/MM/YYYY), computed from UTC fields rather than
    // the validator's own device timezone/clock — so the date the dashboard
    // shows is the same no matter which timezone a validator's phone or PC
    // happens to be set to. nowMs (epoch ms) is always UTC by nature and is
    // what lets the dashboard also show the exact real-time submission time.
    var nowMs = Date.now();
    var nowUTC = new Date(nowMs);
    var pad2 = function(n){ return String(n).length<2 ? '0'+n : String(n); };
    var todayStr = pad2(nowUTC.getUTCDate())+'/'+pad2(nowUTC.getUTCMonth()+1)+'/'+nowUTC.getUTCFullYear();
    var resultNorm = resultValue==='ok' ? 'ok' : 'ng';

    var entry = { round:newRound, date:todayStr, result:resultNorm, validatedBy:validatorName||'', ts:nowMs };
...
// Tolerant match: true when every field the client actually had in
// `original` agrees with `candidate`'s value for that same field (treating
// undefined/null/'' as the same "empty" value, so a column that existed on
// one side but not the other doesn't break the match). Extra fields that
// exist on `candidate` but not on `original` are ignored. This is what lets
// an edit still find its row after the sheet's columns changed shape, while
// staying far more specific than a 3-4 field key (it checks every field the
// client saw — dates, sequence numbers, everything — so two rows that merely
// share the same code/category are still told apart).
function rowMatchesLoosely(candidate, original){
  if(!candidate || !original) return false;
  for (var key in original){
    if(!Object.prototype.hasOwnProperty.call(original,key)) continue;
    var ov = original[key], cv = candidate[key];
    var oEmpty = (ov===undefined || ov===null || ov==='');
    var cEmpty = (cv===undefined || cv===null || cv==='');
...
// Locates the target row in up to three passes:
//  0) by stable row id (_id) — every row gets one the moment it's first
//     uploaded or added (see withRowIds/genRowId above), and it's carried
//     through every save from here on. This is the normal path and it's
//     always exact, even when two rows are otherwise identical (same code,
//     same blank fields, etc.) — content can never disambiguate that, an id
//     always can.
//  1) exact match of the full original snapshot — only reached for a row
//     that predates this id system and hasn't been saved since.
//  2) a tolerant field-by-field match (see rowMatchesLoosely above), for a
//     legacy row whose shape changed server-side since the edit form
//     opened. Only applied when exactly one row in the dataset matches —
//     with zero or more than one we can't safely guess.
// Whichever pass finds the row, the save also stamps it with a stable _id
// (keeping its existing one if it already had one) so any *future* edit to
// that same row goes through the fast, unambiguous pass 0.
// If the row still can't be found at all — someone else edited or
// re-uploaded in the meantime — this throws and the caller should ask the
// person to refresh and retry.
export async function fbUpdateJobRow(originalRow, updatedRow){
  var metaRef = doc(db,'mold_sync','meta');
  var fingerprint = JSON.stringify(originalRow);
  var rowId = originalRow && originalRow._id;
  ...
  return runTransaction(db, async function(tx){
    ...
    function writeAt(chunkIdx, rowIdx, rows, existingId){
      rows[rowIdx] = Object.assign({}, updatedRow, { _id: existingId || rowId || genRowId() });
      tx.set(chunkRefs[chunkIdx], { rows: rows });
      // Bump meta.ts so the live onSnapshot listener (dashboard-admin,
      // readonly) actually refires and re-reads the chunks — without this
      // the edit is saved correctly in Firestore but nobody's open tab
      // ever learns the chunk changed, so it keeps showing whatever was
      // cached before the edit until the page is manually reloaded.
      tx.set(metaRef, Object.assign({}, metaSnap.data(), { ts: Date.now() }));
    }

    // Pass 0: match by stable row id (fast path, always unambiguous).
    if(rowId){
      for (var c0=0; c0<chunkSnaps.length; c0++){
        if(!chunkSnaps[c0].exists()) continue;
        var rows0 = chunkSnaps[c0].data().rows || [];
        var idx0 = rows0.findIndex(function(row){ return row && row._id===rowId; });
        if(idx0>=0){ writeAt(c0, idx0, rows0, rowId); return true; }
      }
    }

    // Pass 1: exact full-row match (for rows saved before ids existed).
    for (var c=0; c<chunkSnaps.length; c++){
      if(!chunkSnaps[c].exists()) continue;
      var rows = chunkSnaps[c].data().rows || [];
      var idx = rows.findIndex(function(row){ return JSON.stringify(row)===fingerprint; });
      if(idx>=0){ writeAt(c, idx, rows, rows[idx]._id); return true; }
    }

    // Pass 2: fall back to a tolerant match, only if it's unambiguous.
    var matches = [];
    for (var c2=0; c2<chunkSnaps.length; c2++){
      if(!chunkSnaps[c2].exists()) continue;
      var rows2 = chunkSnaps[c2].data().rows || [];
      for (var k=0; k<rows2.length; k++){
        if(rowMatchesLoosely(rows2[k], originalRow)) matches.push({chunkIdx:c2, rowIdx:k, rows:rows2});
      }
    }
    if(matches.length===1){
      var m = matches[0];
      writeAt(m.chunkIdx, m.rowIdx, m.rows, m.rows[m.rowIdx]._id);
      return true;
    }
...
// Manually add a single new job row (Data Specialist "add new" form) —
// appends to the last chunk if there's room, otherwise starts a fresh one,
// so this never has to touch/rewrite the rest of the existing data.
export async function fbAddJobRow(newRow){
  newRow = Object.assign({}, newRow, { _id: newRow && newRow._id || genRowId() });
  var metaRef = doc(db,'mold_sync','meta');
  return runTransaction(db, async function(tx){
    var metaSnap = await tx.get(metaRef);
    var chunkCount = metaSnap.exists() ? (metaSnap.data().chunkCount||0) : 0;
    var totalCount = metaSnap.exists() ? (metaSnap.data().totalCount||0) : 0;
    var src = metaSnap.exists() ? (metaSnap.data().src||'') : '';

    if(chunkCount>0){
      var lastRef = doc(db,'mold_sync','chunk_'+(chunkCount-1));
      var lastSnap = await tx.get(lastRef);
      var lastRows = lastSnap.exists() ? (lastSnap.data().rows||[]) : [];
      var testRows = lastRows.concat([newRow]);
      if(byteLen(testRows) <= CHUNK_BYTE_BUDGET){
        tx.set(lastRef, { rows: testRows });
        tx.set(metaRef, { chunkCount: chunkCount, totalCount: totalCount+1, ts: Date.now(), src: src });
        return;
      }
    }
    var newChunkRef = doc(db,'mold_sync','chunk_'+chunkCount);
    tx.set(newChunkRef, { rows: [newRow] });
    tx.set(metaRef, { chunkCount: chunkCount+1, totalCount: totalCount+1, ts: Date.now(), src: src });
  });
}
