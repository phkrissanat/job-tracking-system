...
// Locates the target row in two passes:
//  1) exact match of the full original snapshot against the freshest data
//     (not by array index, which can silently point at the wrong row if
//     anything else changed the dataset between the edit form opening and
//     Save).
//  2) if that misses — e.g. the row's shape changed server-side since this
//     person's edit form was opened (a column added/removed/renamed) — fall
//     back to a tolerant field-by-field match (see rowMatchesLoosely above).
//     This is only applied when exactly one row in the dataset matches;
//     with zero or more than one match we can't safely guess, so it falls
//     through to the not-found error instead.
// If the row still can't be found — someone else edited or re-uploaded in
// the meantime — this throws and the caller should ask the person to
// refresh and retry.
export async function fbUpdateJobRow(originalRow, updatedRow){
  ...
  return runTransaction(db, async function(tx){
    ...
    // Pass 1: exact full-row match (fast path, zero ambiguity).
    for (var c=0; c<chunkSnaps.length; c++){
      if(!chunkSnaps[c].exists()) continue;
    ...
    for (var c2=0; c2<chunkSnaps.length; c2++){
      ...
      for (var k=0; k<rows2.length; k++){
        if(rowMatchesLoosely(rows2[k], originalRow)) matches.push({chunkIdx:c2, rowIdx:k});
      }
    }
    if(matches.length===1){
      var m = matches[0];
      var rows3 = chunkSnaps[m.chunkIdx].data().rows || [];
      rows3[m.rowIdx] = updatedRow;
      tx.set(chunkRefs[m.chunkIdx], { rows: rows3 });
      return true;
    }
