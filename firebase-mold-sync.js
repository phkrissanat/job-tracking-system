...
// Business-identity key for a job row: group+category+code+item. Used as a
// fallback match below when the exact-row match misses — mirrors the same
// helper already used by data-specialist.html's bulk-merge feature.
function jobIdentityKey(r){
  return [r&&r.group, r&&r.category, r&&r.code, r&&r.item].join('|');
}
...
//     back to matching by business identity (group+หมวด+รหัส+รายการ). This
//     is only applied when exactly one row in the dataset has that identity;
...
// the meantime, or the identity itself changed — this throws and the caller
// should ask the person to refresh and retry.
export async function fbUpdateJobRow(originalRow, updatedRow){
  var metaRef = doc(db,'mold_sync','meta');
  var fingerprint = JSON.stringify(originalRow);
  var idKey = jobIdentityKey(originalRow);
  ...
  return runTransaction(db, async function(tx){
    ...
    // Pass 2: fall back to business-identity match, only if it's unambiguous.
    var matches = [];
    for (var c2=0; c2<chunkSnaps.length; c2++){
      ...
      for (var k=0; k<rows2.length; k++){
        if(jobIdentityKey(rows2[k])===idKey) matches.push({chunkIdx:c2, rowIdx:k});
      }
    }
    if(matches.length===1){
      var m = matches[0];
